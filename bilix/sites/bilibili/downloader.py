...
class DownloaderBilibili(BaseDownloaderPart):
    ...
    def parse_url(cls, url: str):
        if re.match(r"https://space\.bilibili\.com/\d+/favlist\?fid=\d+", url):
            ...
            r"https://space\.bilibili\.com/\d+/channel/seriesdetail\?sid=\d+", url
        ):
            return cls.get_collect_or_list
        elif re.match(
            r"https://space\.bilibili\.com/\d+/channel/collectiondetail\?sid=\d+", url
        ):
            return cls.get_collect_or_list
    ...
    async def get_favour(
        ...
        :return:
        """
        fav_name, up_name, total_size, bvids = await api.get_favour_page_info(
            self.client, url_or_fid, keyword=keyword
        )
        if self.hierarchy:
    ...
    async def _get_favor_by_page(
        ...
        ps = 20
        num = min(ps, num)
        _, _, _, bvids = await api.get_favour_page_info(
            self.client, url_or_fid, pn, ps, keyword
        )
        cors = []
        for i in bvids[:num]:
    ...
    async def get_cate(
        ...
        if "subChannelId" not in cate_meta[cate_name]:
            ...
            return self.logger.error(
                f"{cate_name} 是主分区，仅支持子分区，试试 {sub_names}"
            )
        if self.hierarchy:
    ...
    async def _get_cate_by_page(
        ...
        bvids = await api.get_cate_page_info(
            self.client, cate_id, time_from, time_to, pn, 30, order, keyword
        )
        bvids = bvids[:num]
        func = self.get_series if series else self.get_video
        # noinspection PyArgumentList
    ...
    async def get_up(
        ...
        """
        ps = 30
        up_name, total_size, bv_ids = await api.get_up_video_info(
            self.client, url_or_mid, 1, ps, order, keyword
        )
    ...
    async def _get_up_by_page(
        ...
        ps = 30
        num = min(ps, num)
        _, _, bvids = await api.get_up_video_info(
            self.client, url_or_mid, pn, ps, order, keyword
        )
        bvids = bvids[:num]
        func = self.get_series if series else self.get_video
        # noinspection PyArgumentList
    ...
    async def get_video(
        ...
        async with self.v_sema:
            ...
            media_name = (
                base_name
                if not time_range
                else legal_title(base_name, *map(t2s, time_range))
            )
            old_media_name = media_name
            ...
            if video_info.dash:
                try:  # choose video quality
                    ...
                    # 2. video and audio
                        ...
                        old_exists, old_media_path = path_check(
                            path / f"{old_media_name}.mp4"
                        )
                        if old_exists:
                            old_media_path.replace(
                                old_media_path.parent.joinpath(f"{media_name}.mp4")
                            )
                        ...
                        if exists:
                            ...
                            await self.progress.update(
                                task_id=task_id, upper=ffmpeg.combine
                            )
                    # 3. only audio
            ...
            if image or subtitle or dm:
                ...
                if image:
                    add_cors.append(
                        self.get_static(video_info.img_url, path=extra_path / base_name)
                    )
                if subtitle:
                    add_cors.append(
                        self.get_subtitle(url, path=extra_path, video_info=video_info)
                    )
                if dm:
                    try:
                    ...
                    add_cors.append(
                        self.get_dm(
                            url,
                            path=extra_path,
                            convert_func=self._dm2ass_factory(width, height),
                            video_info=video_info,
                        )
                    )
            path_lst, _ = await asyncio.gather(
                asyncio.gather(*media_cors), asyncio.gather(*add_cors)
            )
    ...
    async def get_dm(
        self, url, path=Path("."), update=False, convert_func=None, video_info=None
    ):
        """
        下载视频的弹幕
        :cli: short: dm
        ...
        file_type = "." + (
            "pb" if not convert_func else convert_func.__name__.split("2")[-1]
        )
    ...
    async def get_subtitle(
        self, url, path=Path("."), convert_func=json2srt, video_info=None
    ):
        """
        下载视频的字幕文件
        :cli: short: sub
        ...
        for sub_url, sub_name in subtitles:
            if (
                len(video_info.title) > self.title_overflow
                and self.hierarchy
                and p_name
            ):
            ...
            cors.append(
                self.get_static(sub_url, path / file_name, convert_func=convert_func)
            )
